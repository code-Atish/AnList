...
function ExtLinks() {
  ...
  return (
    <div className="overview_elements">
      <div
        className="pb-5 mb-1em sidebar_det_title"
        ref={(ref) => (window.targetComponentRef = ref)}
      >
        External & Streaming links
      </div>
      <div className="streamlink_wrapper">
        {externalLinks.map(
          ({ url, site, icon, color, notes, language }, index) => (
            <a
              href={url}
              target="_blank"
            ...
            >
              <div
                ...
                className={
                  icon ? "link_det_wrapper" : "link_det_wrapper nolink"
                }
              >
                {icon && (
